...
#ifdef __F32C__
#define PWM_TOP 65000
// **** ATTENTION DIRTY HARDCODED MALLOC,
// works for ulx2s with 1M SRAM,
// allocating upper 512K for DMA buffer
// uint32_t *pcmbuf = (uint32_t *)0x80080000;
// first 128K program, rest for DMA buffer
uint32_t *pcmbuf = (uint32_t *)0x80080000;
uint32_t *pcmptr = pcmbuf; // current address to store PCM value
int pcmsize = 0; // size of the buffer
int16_t pcmvalue = 0; // currently played amplitude
PCM pcm = PCM(); // instantiate PCM DMA
#endif
...
int phonemesToData(const char* textp, const PHONEME* phoneme){
  ...
  while(*textp != 0){
	...
	for(numOut=0; ph_ph[numOut]!= 0; numOut++){
          phonemes[phonemeOut+numOut] = ph_ph[numOut];
        }
      }
      ...
      if(textp[longestMatch]>='0' && textp[longestMatch]<='9'){
	// Pitch change requested
	modifier[modifierOut] = PitchesP[textp[longestMatch]-'1'];
	modifier[modifierOut+1] = L81;
	longestMatch++;
      }
...
void soundOff(void){
  ...
  //DDRB &= ~(_BV(10));		// make B2 an input pin
  pinMode(10,INPUT);
}

void soundOn(void){
  //DDRB |= _BV(10);			// make B2 an output pin
  pinMode(10,OUTPUT);
...
#ifdef __F32C__
void soundOff(void){
  // actually when sound is Off at f32c
