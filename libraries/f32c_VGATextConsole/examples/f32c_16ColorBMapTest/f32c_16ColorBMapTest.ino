...
{
  uint32_t off = (y * FB_WIDTH) + (x>>1);
  if (off < (FB_WIDTH * FB_HEIGHT))
  {
//    color &= 0xf;
    if (x&1)
      fb[off] = (fb[off] & 0x0f) | (color<<4);
    else
      fb[off] = (fb[off] & 0xf0) | (color);
  }
}
...
void loop() {
  ...
  vga.print("Welcome to f32c_16ColorBMapTest");
  vga.SetColor(0x0F);
  vga.println("");
  vga.println("");
  ...
  while (cc++ < 10*60)
  {
	...
	// memory stress test
	  ...
	  while (!VGAText_GetVerticalBlank())
      ;
    for (int i = 0; i < (FB_WIDTH * FB_HEIGHT); i+=4)
    {
        *((volatile uint32_t*)&fb[i]) ^= 0xff55ffaa;
        *((volatile uint32_t*)&fb[i]) ^= 0xff55ffaa;
        *((volatile uint32_t*)&fb[i]) ^= 0xff55ffaa;
        *((volatile uint32_t*)&fb[i]) ^= 0xff55ffaa;
        *((volatile uint32_t*)&fb[i]) ^= 0xff55ffaa;
...
void drawRLE()
{
	...
	while (cmp < &HaDLogo[sizeof(HaDLogo)])
	{
		...
		while (cnt--)
		{
			plot(x, y, curcolor ? (((x>>1)+(y))) & 0x0f : 0x00);
