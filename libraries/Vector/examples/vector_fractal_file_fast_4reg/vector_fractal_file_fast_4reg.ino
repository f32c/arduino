 ...
 * LICENSE=BSD
 *
 * Vector processor test for only 4 vector registers
 * 
...
void fractal_to_file(File &Image)
{
  ...
  // Odd vector numbers are aliases of even vector numbers - both are the same register.
  // Different values are stored in same register using different range.
  Vector_REG Vaz(0), Vbz1(1), Vha(2), Vhb1(3), Vhc(4), Vac1(5), Vbc(6), Vtmp1(7);
  ...
  Vac1.range(2047,2047); // only Mbc at top of Vac
  Vac1 = Mconst; // Vac(2047) contains Mbc
  ...
  Vbz1.range(SCREEN_WIDTH, 2*SCREEN_WIDTH-1);
  Vha.range(0, SCREEN_WIDTH-1);
  Vhb1.range(SCREEN_WIDTH, 2*SCREEN_WIDTH-1);
  Vhc.range(0, SCREEN_WIDTH-1);
  Vac1.range(SCREEN_WIDTH, 2*SCREEN_WIDTH-1);
  Vbc.range(0, SCREEN_WIDTH-1);
  Vtmp1.range(SCREEN_WIDTH, 2*SCREEN_WIDTH-1);
  ...
  for(y = 0; y < SCREEN_HEIGHT; y++)
  {
    ...
    for(i = ITERATIONS-1; i >= 0; i--)
    {
      ...
      Vaz = Vtmp1 + Vac1;
      Vtmp1 = Vhc + Vhc;
      Vac1.range(2047,2047); // vbc is constant at the top of vac
      Vbz1 = Vtmp1 + Vac1; // add vbc constant
      Vac1.range(SCREEN_WIDTH, 2*SCREEN_WIDTH-1); // range back to normal
      Vha = Vaz * Vaz;
      Vhb1 = Vbz1 * Vbz1;
      if(i == 1)
      {
    ...
    Vtmp1 = Mred;
    Vaz = Vha-Vtmp1;
    Vha.range(2041,2041); // range to red scale constant
    Vbz1.range(0, SCREEN_WIDTH-1); // Vbz1 becomes Vaz1 alias
    Vaz = Vbz1 * Vha; // *constant
    Vbz1.range(SCREEN_WIDTH, 2*SCREEN_WIDTH-1); // back to normal
    Mred = Vaz;
    // green channel
    Vbz1 = Mgreen;
    Vaz.range(SCREEN_WIDTH, 2*SCREEN_WIDTH-1); // Vaz becomes Vbz1 alias
    Vbz1 = Vaz-Vhb1; // Vbz=Vbz+Vhb
    Vha.range(2042,2042); // range to green scale constant
    Vbz1 = Vaz * Vha; //  Vbz1 *= constant
    Vaz.range(0, SCREEN_WIDTH-1); // back2normal
    Mgreen = Vbz1;
    // blue channel
    Vha.range(0, SCREEN_WIDTH-1); // back2nomal
    Vaz = Vha + Vhb1;
    Vha.range(2043,2043); // range to blue scale constant
    Vtmp1 = Vaz * Vha; // *constant
    Vha.range(0, SCREEN_WIDTH-1); // back2nomal
    Mblue = Vtmp1;
    ...
    Vha.range(2047,2047); // range to constant for Vbc increment
    Vhc.range(2047,2047); // Vhc is alias of vac1, Mbc is here at top, Vac(2047)
    Vac1.range(2047,2047);
    Vac1 = Vhc + Vha; // increment
    Vac1.range(SCREEN_WIDTH, 2*SCREEN_WIDTH-1);
    Vha.range(0, SCREEN_WIDTH-1); // range back to normal
