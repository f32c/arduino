/************************************************* 
   ...
   Easter Egg: Slide filenames which end with
   "*sprit.ppm" or "*SPRIT.PPM" are treated as "special".
   Instead of the slide, a sprite animation is shown.
 *************************************************/
...
void read_slides_dir(char *directory_path)
{
   ...
   for(int i = 0; i < DirEntries_count; i++)
   {
     ...
     if(sprite_index >= 0)
     {
       ...
       #if 1
       if(filename.endsWith("sprit.ppm") ||
          filename.endsWith("SPRIT.PPM") )
       {
         Sprite_slide = i;
         Serial.print("Sprite_slide=");
         Serial.println(Sprite_slide);
       }
       #endif
...
void background_reader()
{
  ...
  if(file_open == 0)
  {
    ...
    file_open = 1;
    digitalWrite(8, reading_slide & 1);
...
void loop()
{
  ...
  if(Slide_shown != Old_Slide_shown)
  {
    ...
    if(Old_Slide_shown == Sprite_slide)
      stop_sprites();
    if(Slide_shown == Sprite_slide)
      prepare_sprites();
    else
